                    ...
                    <div class="box">
                        <div class="box-body">
                            <div class="d-md-flex d-block align-items-center justify-content-between">
                                ...
                                <a href="{{route('client.invest_withdrawal')}}" class="btn btn-info">
                                    <i class="fa fa-download me-10"></i>Faire un retrait
                                </a>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody class="text-fade">
                                        @foreach ($withdrawals as $withdrawal)
                                            @php
                                                $client = \App\Models\Client::find($withdrawal->user_id);
                                            @endphp
                                            <tr>
                                                <th scope="row">TRANSACT.<span style="text-info">{{ ++$nb }}</span></th>
                                                <td>{{ $withdrawal->price_amount }} {{ $withdrawal->price_currency }}</td>
                                                <td>{{ $withdrawal->to_account }}</td>
                                                <td>{{ $withdrawal->created_at->format('d/m/Y à H:i') }}</td>
                                                <td>{{ $client ? $client->last_name . ' ' . $client->first_name : 'N/A' }}</td>
